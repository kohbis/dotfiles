...
`coding-with-codex`, `reviewing-with-codex`, `running-copilot-cli`, `reviewing-with-multi-models`. Other skills under `agents/skills/` don't pin model names and can be ignored unless a search turns them up.
...
```bash
rg -n --no-heading -- 'gpt-[0-9]|claude-(sonnet|opus|haiku|[0-9])|--model ' \
